...
const _DEFAULT_TEXTURE = "res://Engine/Assets/Trail/texture.tres"
const _DEFAULT_CURVE = "res://Engine/Assets/Trail/curve.tres"
...
func _ready():
	if not _defaults_have_been_set:
		...
		process_material.shader = preload("res://Engine/Assets/Trail/trail.gdshader")

		draw_pass_1 = QuadMesh.new()
		draw_pass_1.material = ShaderMaterial.new()
		draw_pass_1.material.shader = preload("res://Engine/Assets/Trail/trail_draw_pass.gdshader")
...
func _process(delta):
	...
	RenderingServer.instance_geometry_set_shader_parameter(get_instance(), "instancePos", global_transform.origin)
	RenderingServer.instance_geometry_set_shader_parameter(get_instance(), "instanceRot", global_transform.basis.get_euler())
